...
// Qt 6.10.3: QCryptographicHash::hash tiene una race interna cuando N hilos
// hashean a la vez (los 9 logins paralelos del refreshAll/spawn llaman
// md5Hex/buildMidPem simultaneamente) -> AV 0xC0000005 en Qt6Core+0x1CE857
// (familia SHA-512, faulting address [r12+0x78] con r12~0). Se serializa con
// un mutex global: el costo es microsegundos por hash, irrelevante frente a
// los ~500ms de cada HTTP del login.
static QMutex g_qtHashMutex;

// Mutex global del parseo JSON de los hilos de trabajo (refreshAll/spawn):
// QJsonDocument::fromJson + QVariantMap/QHash internos llaman qHashBits de
// Qt 6.10.3 (SHA-512 interno con race cuando N hilos hashean a la vez ->
// AV 0xC0000005 en Qt6Core+0x1CE857). Las funciones de parseo de login.cpp
// y tcp_farm.cpp lo toman por llamada (~1-5ms cada una: el flujo HTTP sigue
// en paralelo, solo el parseo se serializa).
QMutex g_jsonParseMutex;

// Ver crypto.h: serializa el login completo (KNOCK/LIM/EH + lecturas) entre
// los hilos de pre-spawn/refreshAll/QWS/farm. La race de Qt 6.10.3 en la
// familia qHashBits/SHA-512 sobrevive a los mutex de parseo/hash porque las
// CONSTRUCCIONES de QJsonObject/QVariantMap tambien hashean.
QMutex g_loginMutex;

// Serializa el SPAWN TCP completo (spawnSession: greeting/AUTH/IRC/LISTENER/
// handshake/proof). Los 9 farms spawneando a la vez crasheaban Qt 6.10.3
// (AV 0x1CF461/0x1CF4A4, NULL-base) en internals que no pasan por el mutex
// de login (QNetworkRequest/QSet/QHash de los handlers del handshake).
// Con esto el arranque de N farms es secuencial (~10s por spawn) pero el
// loop de farmeo posterior corre libre (el lock solo cubre el spawn).
QMutex g_spawnMutex;
// 2026-08-10 (test #44/#45): el server SOLO tolera UNA cuenta en espera del
// [20] SPAWNED por IP (corta a los ~8s si hay mas). g_matchMutex serializa
// SOLO esa espera, dejando el handshake (g_spawnMutex) libre para las demas:
// antes el lock unico se retenia hasta el [20] y las ultimas cuentas
// tardaban minutos en entrar (Java 2m40s en #45).
QMutex g_matchMutex;
QMutex g_refreshFullMutex;
...
{
    // mutex: QCryptographicHash::hash no es seguro con hilos paralelos (Qt 6.10.3)
    QMutexLocker locker(&g_qtHashMutex);
...
Bytes rsaSignPkcs1Sha256(const QString &pem, const QByteArray &msg)
{
    ...
    QMutexLocker hashLocker(&g_qtHashMutex); // QCryptographicHash: no thread-safe en Qt 6.10.3
...
static Bytes bigMod(const Bytes &a, const Bytes &m)
{
    return bigDivMod(a, m).second;
}

// desplaza un big-endian number 1 bit a la derecha (divide por 2)
static Bytes bigShr1(Bytes v)
{
    v = bigTrim(v);
    if (v.empty())
        return v;
    for (size_t i = 0; i + 1 < v.size(); ++i)
        v[i] = std::uint8_t((v[i] >> 1) | ((v[i + 1] & 1) ? 0x80 : 0));
    v.back() = std::uint8_t(v.back() >> 1);
    return bigTrim(v);
}

// (base ^ exp) mod mod, square-and-multiply. Fermat: a^{-1} = a^{m-2} (mod m)
static Bytes bigModPow(const Bytes &base, const Bytes &exp, const Bytes &mod)
{
    Bytes result{1};
    Bytes b = bigMod(base, mod);
    Bytes e = bigTrim(exp);
    while (!(e.size() == 1 && e[0] == 0)) {
        if (e.back() & 1)
            result = bigMod(bigMul(result, b), mod);
        e = bigShr1(e);
        b = bigMod(bigMul(b, b), mod);
    }
    return result;
}
...
static Bytes derivePrivateExponent(const Bytes &p, const Bytes &q, const Bytes &e)
{
    ...
    if (invPhi == 0) {
        QFile dbg(QCoreApplication::applicationDirPath() + QStringLiteral("/keygen_debug.txt"));
        if (dbg.open(QIODevice::Append | QIODevice::Text)) {
            QTextStream ts(&dbg);
            ts << "derive: invPhi=0 phiMod=" << phiMod << " emod=" << emod
               << " phi_sz=" << phi.size() << " p_sz=" << p.size() << " q_sz=" << q.size() << "\n";
            dbg.close();
        }
        return Bytes();
    }
    ...
    if (!(dr.second.size() == 1 && dr.second[0] == 0)) {
        QFile dbg(QCoreApplication::applicationDirPath() + QStringLiteral("/keygen_debug.txt"));
        if (dbg.open(QIODevice::Append | QIODevice::Text)) {
            QTextStream ts(&dbg);
            ts << "derive: rem != 0 rem_sz=" << dr.second.size()
               << " num_sz=" << num.size() << " e_sz=" << e.size()
               << " phiMod=" << phiMod << " invPhi=" << invPhi << " k=" << k << "\n";
            dbg.close();
        }
        return Bytes();
    }
...
bool generateRsaPem2048(QString *pemOut)
{
    ...
    try {
        ...
        DWORD blobLen = 0;
        requireBcrypt(BCryptExportKey(key, nullptr, BCRYPT_RSAFULLPRIVATE_BLOB, nullptr, 0,
                                      &blobLen, 0), "ExportKey(size)");
        Bytes blob(blobLen);
        requireBcrypt(BCryptExportKey(key, nullptr, BCRYPT_RSAFULLPRIVATE_BLOB, blob.data(),
                                      DWORD(blob.size()), &blobLen, 0), "ExportKey");
        blob.resize(blobLen);

        // BCRYPT_RSAFULLPRIVATE_BLOB (RSA7): header + e + n + p + q + dp + dq +
        // qinv + d. (RSA2 en este Windows SOLO devuelve e,n,p,q sin CRT, por
        // eso se usa RSA7: el DER necesita d, dp, dq y qinv.)
        ...
        if (blob.size() < sizeof(BCRYPT_RSAKEY_BLOB)) { fail("blob too small"); return false; }
        ...
        Bytes d = take(hdr.cbModulus);
        if (e.empty() || n.empty() || p.empty() || q.empty()
            || dp.empty() || dq.empty() || qinv.empty() || d.empty()) {
            fail("empty component (RSA7)");
            return false;
        }
        // BCrypt exige p > q en su blob privado: si vienen al reves, invertir
        // p/q y dp/dq (qinv = q^-1 mod p queda correcto con p > q).
        if (bigCmp(p, q) < 0) {
            std::swap(p, q);
            std::swap(dp, dq);
        }
