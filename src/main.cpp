...
int main(int argc, char *argv[])
{
    ...
    if (hasArg(args, QStringLiteral("--headless-run"))) {
        ...
        // --account "Nombre1,Nombre2" separadas por coma) y las spawnea
        ...
        const QStringList accountParts = accountValue.split(QLatin1Char(','), Qt::SkipEmptyParts);
        QTimer::singleShot(3000, &farm, [&farm, accountParts]() {
            const auto accs = farm.accounts();
            for (int i = 0; i < accs.size(); ++i) {
                bool match = true;
                if (!accountParts.isEmpty()) {
                    match = false;
                    const QVariantMap m = accs[i].toMap();
                    for (const QString &part : accountParts) {
                        const QString p = part.trimmed();
                        if (m.value(QStringLiteral("name")).toString()
                                .contains(p, Qt::CaseInsensitive)
    ...
    if (durationSecs > 0) {
        // v97co (dato del usuario: al cumplir el tiempo el bot debe CORTAR la
        // conexion y esperar ~30s para re-loguear — la cuenta muere en el
        // server y el cexp materializa; el login nuevo capta la XP REAL):
        // stopFarm a los Ns -> esperar 30s (materializacion del server) ->
        // fetchAllGems (login nuevo que persiste el cexp final) -> cerrar.
        QTimer::singleShot(durationSecs * 1000, &farm, [&farm]() { farm.stopFarm(); });
        QTimer::singleShot(durationSecs * 1000 + 30000, &farm, [&farm]() { farm.fetchAllGems(); });
        QTimer::singleShot(durationSecs * 1000 + 45000, &farm, [&farm]() { farm.quitApp(); });
