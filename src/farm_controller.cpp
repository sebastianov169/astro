...
void FarmController::appendLog(const QString &line, bool writeFile)
{
    {
        QMutexLocker locker(&m_debugMutex);
    ...
    // v97cm (pedido del usuario: "mejora muchisimo mas el log"): las lineas
    // del WORKER ya se escriben con el prefijo [Cuenta]+ms en el worker
    // (writeWorkerLog) — el appendLog del stateChanged NO debe duplicarlas.
    if (writeFile)
        writeLogFile(line);
...
void FarmController::fetchAllGems()
{
    ...
    connect(thread, &QThread::started, thread, [this, thread, targets]() {
        ...
        struct FetchOut {
            ...
            int currentId = -1; // v97cg: data.current REAL del inventory (la gema EQUIPADA)
        };
        ...
        for (int k = 0; k < targets.size(); ++k) {
            ...
            o.currentId = local.lastCurrentItem(); // v97cg: la gema EQUIPADA real
        ...
        QMetaObject::invokeMethod(this, [this, thread, targets, outs]() {
            for (int k = 0; k < targets.size(); ++k) {
                ...
                for (int i = 0; i < m_accounts.size(); ++i) {
                    ...
                    if (!o.gems.isEmpty()) {
                        // v97cg (bug de la barra con varias gemas: el fetchAll
                        // usaba o.gems.first() — la PRIMERA gema de la lista,
                        // que NO es necesariamente la equipada. El usuario la
                        // vio desfasada: Chasing barra 2956 vs current 3942).
                        // Usar la gema del data.current real; fallback a la
                        // primera si el current no esta en la lista.
                        const GemInfo *egPtr = nullptr;
                        for (const GemInfo &g : o.gems) {
                            if (g.id == o.currentId) { egPtr = &g; break; }
                        }
    ...
    for (int k = 0; k < total; ++k) {
        ...
        connect(thread, &QThread::started, thread, [this, thread, deviceId, pemPath, k, localId, preps, done, total, priorityColors]() {
            ...
            if (r.ok) {
                ...
                // v97bv (SOLO app de TESTS): auto-buy restaurado para poder
                // testear — si el slot 5 esta vacio O TODAS las gemas son
                // LVL25 (prohibidas), comprar la primera gema comprable del
                // shop y equiparla. La app ESTABLE mantiene el v97af.
                bool allLvl25 = !prep.gems.isEmpty();
                for (const GemInfo &g : prep.gems)
                    if (g.itemLevel < 25) { allLvl25 = false; break; }
                if ((prep.gems.isEmpty() || allLvl25) && !priorityColors.isEmpty()) {
                    writeLogFile(QStringLiteral("[DBG] spawnLogin %1 sin gema o solo lvl25 - auto-buy de TESTS (v97bv)")
                                     .arg(logName(deviceId, prep.realName)));
                    const QVector<StoreItem> shop = local.fetchStore(10);
                    for (const auto &si : shop) {
                        if (si.id <= 0 || !si.purchasable || si.price <= 0)
                            continue;
                        QJsonObject buy = local.apiCall(QStringLiteral("{\"do\":\"buy\",\"item\":%1}").arg(si.id));
                        if (buy.value("result").toString() != QLatin1String("ok"))
                            continue;
                        QJsonObject eq = local.apiCall(QStringLiteral("{\"do\":\"equip\",\"item\":%1,\"slot\":5}").arg(si.id));
                        writeLogFile(QStringLiteral("[DBG] spawnLogin %1 auto-buy %2 (%3) -> %4")
                                         .arg(logName(deviceId, prep.realName)).arg(si.id).arg(si.name)
                                         .arg(eq.value("result").toString()));
                        prep.gems = local.fetchInventory(5);
                        break;
                    }
                }
                ...
                prep.ok = true;
                // v97cf (bug del contador con gemas cambiadas despues del
                // arranque: el refresh comparaba contra el cache VIEJO del
                // accounts.json -> delta desfasado): re-scan del login —
                // persistir AQUI el cexp/exp y el cache de gemas ACTUALES
                // (el baseline real del farm que empieza ahora).
                for (auto &av : m_accounts) {
                    QVariantMap am = av.toMap();
                    if (am.value(QStringLiteral("device")).toString() != deviceId)
                        continue;
                    const GemInfo *eqPtr = nullptr;
                    for (const GemInfo &g : prep.gems) {
                        if (g.id == prep.equippedId) { eqPtr = &g; break; }
                    }
                    if (eqPtr) {
                        am.insert(QStringLiteral("cexp"), qlonglong(eqPtr->cexp));
                        am.insert(QStringLiteral("exp"), qlonglong(eqPtr->exp));
                        am.insert(QStringLiteral("equippedGemId"), eqPtr->id);
                        // v97cf: la barra del dashboard usa el nivel/nombre/
                        // sprite — actualizarlos tambien (igual que el refresh).
                        am.insert(QStringLiteral("gemLevel"), eqPtr->itemLevel);
                        am.insert(QStringLiteral("gemSummary"),
                                  QStringLiteral("%1 Lv%2").arg(translateGemName(eqPtr->name)).arg(eqPtr->itemLevel));
                        am.insert(QStringLiteral("sprite"), gemSpritePath(eqPtr->name, eqPtr->itemLevel));
                    }
                    am.insert(QStringLiteral("gems"), gemsCacheFrom(prep.gems));
                    av = am;
                    break;
                }
                saveAccounts();
                // v97cf: avisar al dashboard (hilo principal) para que la UI
                // repinte la gema/nombre/barra con los datos del scan — sin
                // esto el QML seguiria mostrando la gema vieja hasta el
                // proximo refresh.
...
void FarmController::onFarmState(FarmWorker *w, const QString &text)
{
    ...
    // Log legible (sin prefijo [DBG]) — v97cm: sin re-escribir al archivo
    // (el worker ya lo escribio con su prefijo [Cuenta]+ms)
    appendLog(QStringLiteral("[%1] %2").arg(prefix, text), false);
}
...
void FarmController::onGemXpRead(FarmWorker *w, qlonglong cexp, qlonglong exp)
{
    ...
    // v97cp (reversion del v97ch — dato del usuario: la gema en 0 con la
    // barra en 3193 y el +2450 = el baseline lo PISO una lectura en vivo):
    // el onGemXpRead NO debe persistir/repintar — cada lectura (60s/equip
    // check) corrompia el cexp del accounts.json y el refresh comparaba
    // contra un baseline a medias. El dashboard SOLO se actualiza con el
    // login (v97cf) y el refresh/autorefresh.
    scheduleUiFlush();
}
...
void FarmController::maybeStartRefreshAllLogin()
{
    ...
    for (const auto &v : m_accounts) {
        const QVariantMap m = v.toMap();
        // v97ci (pedido del usuario: el autorefresh NO debe loguear las 9
        // cuentas — solo las MARCADAS en la lista de workflow/farm).
        const QString dev = m.value(QStringLiteral("device")).toString();
        if (!m_farmSelection.isEmpty() && !m_farmSelection.contains(dev))
            continue;
        RefreshTarget t;
        t.deviceId = dev;
